      ...
      <div className="flex-1 container mx-auto px-4 py-24">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 space-y-6">
            ...
            {(!requests || requests.length === 0) ? (
              <div className="text-center py-20 bg-muted/50 rounded-xl border border-border">
                <p className="text-muted-foreground mb-4">暂无悬赏任务</p>
              ...
              <div className="grid gap-4">
                {requests.map((req: any) => (
                  <Link href={`/requests/${req.id}`} key={req.id}>
                    <div className="bg-card border border-border rounded-xl p-6 hover:border-primary/50 transition-all hover:shadow-lg hover:shadow-primary/5 group relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex gap-4">
                            ...
                            <div>
                                <h3 className="text-lg font-bold group-hover:text-primary transition-colors flex items-center gap-2">
                                    {req.title}
                                    {req.status === 'closed' && (
                                        <Badge variant="secondary" className="bg-secondary text-secondary-foreground">已结束</Badge>
                                    )}
                                </h3>
                        ...
                        <div className="flex flex-col items-end gap-1">
                            <div className="flex items-center text-yellow-600 dark:text-yellow-400 font-bold text-lg">
          ...
          <div className="w-full md:w-80 space-y-6">
            <div className="bg-gradient-to-br from-blue-600/10 to-purple-600/10 border border-blue-500/20 rounded-xl p-6 backdrop-blur-sm sticky top-24 dark:from-blue-600/20 dark:to-purple-600/20">
                <h3 className="text-xl font-bold mb-2 text-foreground">有需求？</h3>
            ...
            <div className="bg-card border border-border rounded-xl p-6">
                <h3 className="font-bold mb-4 flex items-center gap-2 text-card-foreground">
                    ...
                    {requests
                        ...
                        <Link href={`/requests/${req.id}`} key={req.id} className="block group">
                            <div className="flex justify-between items-center mb-1">
                                <span className="text-sm text-muted-foreground group-hover:text-primary truncate flex-1 mr-2 transition-colors">
                                    {req.title}
                                </span>
                                <span className="text-yellow-600 dark:text-yellow-400 text-xs font-mono font-bold">
                                    ¥{req.budget}
                                </span>
                            </div>
